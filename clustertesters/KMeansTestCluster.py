...
class KMeansTestCluster():
    def __init__(self, X, y, clusters, plot=True, targetcluster=2, stats=False):
        self.X = X
        self.y = y
    ...
    def run(self):
        ...
        for k in self.clusters:
            ...
            accuracy.append(metrics.accuracy_score(self.y, labels))
        print (meandist)
        if self.gen_plot:
            self.visualize()

            self.plot(meandist, homogeneity_scores, completeness_scores, rand_scores, silhouettes,accuracy)
    ...
    def visualize(self):
        ...
        X_new = decomposition.PCA(n_components=3).fit_transform(self.X)
        ...
        ax.set_title("Breast Cancer K-Means Clustering")
        ax.set_yticklabels([])
        ax.set_xticklabels([])
        ax.set_zticklabels([])
        plt.show()

    def plot(self, meandist, homogeneity, completeness, rand, silhouettes, accuracy):
            """
            ...
            plt.title('Breast Cancer K-Means: Average Within Cluster SSE vs Number of Clusters')
            plt.show()
            ...
            plt.title('Breast Cancer K-Means: Homogeneity vs Number of Clusters')
            #plt.show()
            ...
            plt.title('Breast Cancer K-Means: Completeness vs Number of Clusters')
            plt.show()
            ...
            ax = plt.figure().gca()
            ax.xaxis.set_major_locator(MaxNLocator(integer=True))
            plt.plot(self.clusters, accuracy)
            plt.xlabel('Number of clusters')
            plt.ylabel('Accuracy')
            plt.title('Breast Cancer K-Means: Accuracy vs Number of Clusters')
            plt.show()
            ...
            plt.title('Breast Cancer K-Means: Adjusted Rand Score vs Number of Clusters')
            plt.show()
